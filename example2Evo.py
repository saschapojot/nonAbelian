...
def jcExpression(t):
    if 0<=t and t<= T/3:
        return r*np.sin(alpha+(np.pi/2-alpha)/(T/3)*t)
    elif T/3<t and t<=2/3*T:
        return r*np.sin(np.pi/2+(alpha-np.pi/2)/(T/3)*(t-T/3))
    else:
        return r*np.sin(alpha)

def jdExpression(t):
    return jcExpression(t)
...
def jb1Expression(t):
    if 0<=t and t<=T/3:
        ...
        return r*np.cos(np.pi/2+(alpha-np.pi/2)/(T/3)*(t-T/3))
    else:
        return r*np.cos((np.pi/2)/(T/3)*(t-2/3*T))*np.cos(alpha)

def jb2Expression(t):
    if 0 <= t and t <= T / 3:
        return r*np.cos(alpha+(np.pi/2-alpha)/(T/3)*t)

    elif T / 3 < t and t <= 2 / 3 * T:
        return 0
    else:
        return r*np.sin((np.pi/2)/(T/3)*(t-2/3*T))*np.cos(alpha)
