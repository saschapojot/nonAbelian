...
tauS=sp.symbols("tau",cls=sp.Symbol)
from sympy import pi as PI
jcS=r*sp.cos(2*PI*tauS)
jdS=r*sp.sin(2*PI*tauS)*sp.cos(alpha1+2*PI*tauS)
jb1S=r*sp.sin(2*PI*tauS)*sp.sin(alpha1+2*PI*tauS)*sp.cos(alpha2+2*PI*tauS)
jb2S=r*sp.sin(2*PI*tauS)*sp.sin(alpha1+2*PI*tauS)*sp.sin(alpha2+2*PI*tauS)
dtauJcSymbol=sp.diff(jcS,tauS)
dtauJdSymbol=sp.diff(jdS,tauS)
dtauJb1Symbol=sp.diff(jb1S,tauS)
dtauJb2Symbol=sp.diff(jb2S,tauS)
#########lambdify
###
jcLambdified=sp.lambdify(tauS,jcS,"numpy")
jdLambdified=sp.lambdify(tauS,jdS,"numpy")
jb1Lambdified=sp.lambdify(tauS,jb1S,"numpy")
jb2Lambdified=sp.lambdify(tauS,jb2S,"numpy")
###
dtauJcLambdified=sp.lambdify(tauS,dtauJcSymbol,"numpy")
dtauJdLambdified=sp.lambdify(tauS,dtauJdSymbol,"numpy")
dtauJb1Lambdified=sp.lambdify(tauS,dtauJb1Symbol,"numpy")
dtauJb2Lambdified=sp.lambdify(tauS,dtauJb2Symbol,"numpy")
##
def jcExpression(tau):
    return jcLambdified(tau)

def jdExpression(tau):
    return jdLambdified(tau)

def dtaujcExpression(tau):
    return dtauJcLambdified(tau)

def dtaujdExpression(tau):
    return dtauJdLambdified(tau)

def jb1Expression(tau):
    return jb1Lambdified(tau)

def jb2Expression(tau):
    return jb2Lambdified(tau)

def jbExpression(k,tau):
    return jb1Expression(tau)+np.exp(1j*k)*jb2Expression(tau)

def dtaujb1Expression(tau):
    return dtauJb1Lambdified(tau)


def dtaujb2Expression(tau):
    return dtauJb2Lambdified(tau)
...
driftTopological=[elem-positionTopologicalAtEachInstant[0] for elem in positionTopologicalAtEachInstant]
...
ibcDrift=[elem-ibcAtEachInstant[0] for elem in ibcAtEachInstant]
outDir="./"+name+"Evo/"
...
pdOut.to_csv(outDir+"ibc.csv",index=False)