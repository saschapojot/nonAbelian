...
def jcExpression(tau):
    if 0<=tau and tau<=1/3:
        return r*np.sin(alpha+3*(np.pi/2-alpha)*tau)
    elif 1/3<tau and tau<=2/3:
        return r*np.sin(np.pi/2+3*(alpha-np.pi/2)*(tau-1/3))
    else:
        return r*np.sin(alpha)

def jdExpression(tau):
    return jcExpression(tau)

def dtaujcExpression(tau):
    if 0 <= tau and tau <= 1 / 3:
        return r*np.cos(alpha+3*(np.pi/2-alpha)*tau)*3*(np.pi/2-alpha)
    elif 1 / 3 < tau and tau <= 2 / 3:
        return r*np.cos(np.pi/2+3*(alpha-np.pi/2)*(tau-1/3))*3*(alpha-np.pi/2)
    else:
        return 0

def dtaujdExpression(tau):
    return dtaujcExpression(tau)

def jb1Expression(tau):
    if 0 <= tau and tau <= 1 / 3:
        return 0
    elif 1 / 3 < tau and tau <= 2 / 3:
        return r*np.cos(np.pi/2+3*(alpha-np.pi/2)*(tau-1/3))
    else:
        return r*np.cos(3/2*np.pi*(tau-2/3))*np.cos(alpha)

def jb2Expression(tau):
    if 0 <= tau and tau <= 1 / 3:
        return r*np.cos(alpha+3*(np.pi/2-alpha)*tau)
    elif 1 / 3 < tau and tau <= 2 / 3:
        return 0
    else:
        return r*np.sin(3*np.pi/2*(tau-2/3))*np.cos(alpha)
...
def dtaujb1Expression(tau):
    if 0 <= tau and tau <= 1 / 3:
        ...
        return -r*np.sin(np.pi/2+3*(alpha-np.pi/2)*(tau-1/3))*3*(alpha-np.pi/2)
    else:
        return -r*np.sin(3/2*np.pi*(tau-2/3))*3/2*np.pi*np.cos(alpha)
def dtaujb2Expression(tau):
    if 0 <= tau and tau <= 1 / 3:
        return -r*np.sin(alpha+3*(np.pi/2-alpha)*tau)*3*(np.pi/2-alpha)
    elif 1 / 3 < tau and tau <= 2 / 3:
        return 0
    else:
        return r*np.cos(3/2*np.pi*(tau-2/3))*3/2*np.pi*np.cos(alpha)
